...
{var obj=ca.__object__;var co=ca.getContext('2d');var color=arguments[1]||(obj&&obj.get('clearto'));if(!ca){return;}
...
{var ca=canvas=obj.canvas;var co=context=obj.context;var prop=obj.properties;if(arguments[5]){var ca=canvas=arguments[5].canvas;var co=context=arguments[5].context;}
...
{var ca=canvas;var co=context;var prop=obj.properties;var height=0;var gutterLeft=obj.gutterLeft;var gutterRight=obj.gutterRight;var gutterTop=obj.gutterTop;var gutterBottom=obj.gutterBottom;var variant=prop['chart.variant'];co.fillStyle=prop['chart.text.color'];if(variant=='3d'){co.save();co.translate(10,-5);}
...
RG.DrawTitle(obj,prop['chart.title'],gutterTop,null,prop['chart.title.size']?prop['chart.title.size']:prop['chart.text.size']+2,{canvas:ca,context:co});}
co.stroke();}
...
{if(!RG.cache[id]){RG.cache[id]={};RG.cache[id].object=obj;RG.cache[id].canvas=$('<canvas></canvas>').attr({width:obj.canvas.width,height:obj.canvas.height,id:'background_cached_canvas'+obj.canvas.id}).get(0);if(typeof G_vmlCanvasManager==='object'&&G_vmlCanvasManager.initElement){G_vmlCanvasManager.initElement(RG.cache[id].canvas);}
...
{var i;for(key in config){var isObject=false;var isArray=false;var value=config[key]
...
return settings;};var settings=recurse(obj,config,'chart',{});for(key in settings){if(typeof key==='string'){obj.set(key,settings[key]);}}};})(window,document);window.$p=function(v)
